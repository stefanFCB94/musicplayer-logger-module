...
import { log_debug, Logger } from '../src';
...
describe('Log debug message', () => {
  ...
  it('should log the message with debug level', (done) => {
    log_debug('123', 'abc');

    moxios.wait(() => {
      expect(JSON.parse(moxios.requests.mostRecent().config.data).level).toEqual('debug');
      done();
    });
  });
