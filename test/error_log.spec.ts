...
import { log_error, Logger } from '../src';
...
describe('Log error message', () => {
  ...
  it('should log the message with error level', (done) => {
    log_error('123', 'abc');

    moxios.wait(() => {
      expect(JSON.parse(moxios.requests.mostRecent().config.data).level).toEqual('error');
      done();
    });
  });
