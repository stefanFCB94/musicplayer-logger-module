...
import { log_info, Logger } from '../src';
...
describe('Log info message', () => {
  ...
  it('should log the message with info level', (done) => {
    log_info('123', 'abc');

    moxios.wait(() => {
      expect(JSON.parse(moxios.requests.mostRecent().config.data).level).toEqual('info');
      done();
    });
  });
