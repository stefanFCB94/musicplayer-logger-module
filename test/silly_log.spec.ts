...
import { log_silly, Logger } from '../src';


describe('Log silly message', () => {
  ...
  it('should log the message with silly level', (done) => {
    log_silly('123', 'abc');

    moxios.wait(() => {
      expect(JSON.parse(moxios.requests.mostRecent().config.data).level).toEqual('silly');
      done();
    });
  });
