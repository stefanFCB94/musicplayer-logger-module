...
import { log_verbose, Logger } from '../src';


describe('Log verbose message', () => {
  ...
  it('should log the message with verbose level', (done) => {
    log_verbose('123', 'abc');

    moxios.wait(() => {
      expect(JSON.parse(moxios.requests.mostRecent().config.data).level).toEqual('verbose');
      done();
    });
  });
